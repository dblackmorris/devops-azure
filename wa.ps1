...
#New-AzResourceGroup -Name testabc123 -Location northcentralus(if new resource group is required)
    ...
    if((Get-AzurermResource -Name $siteName -ResourceType Microsoft.Web/sites).Name -eq $siteName){
        Write-Host ("Resource with the given name exists: "+$siteName)
     }
        ...
        Write-Host ("Resource group does not exist. Creating new resource group: " +$rg)
         ...
         Write-Host ("Resource created: " +$siteName)
         break
        }
